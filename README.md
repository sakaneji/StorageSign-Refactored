...
外部 Logger は任意で、未導入でも標準ロガーへフォールバックします。新しい Minecraft 版で保存・更新したワールドを古い版で開くダウングレードは非対応です。
...
| 項目 | 要件 |
|---|---|
| サーバーソフト | Paper 1.21.4 / 1.21.8 / 1.21.11 / 26.1.2 / 26.2（26.xは実サーバー未完走のため保留） |
| Java | 21 以降 |
...
- 同じ内容を持つ登録済みStorageSignアイテムを右クリックすると、看板一枚単位で内容をマージします。
  上限まで一枚分が収まらない場合でも、入る分だけをマージします。複数枚スタックしているときは、
  空StorageSignを先に返し、その後に残量が減ったStorageSignを返します。インベントリ満杯時はその順序のまま
  足元へドロップし、未マージの中身を通常アイテムとしてドロップすることはありません。
...
不吉な旗の Bukkit API 構築に失敗した場合は白旗へ変換せず、数量も変更しません。1 ティック後から 5 秒間隔でメタ復旧を再試行し、復旧するまでは不吉な旗の搬出を停止します。
...
候補チャンクは実ブロック確認のためにロードされます。前面方向が保存済み索引にもロード済み看板にもない場合はワープしません。ワープ先は前面ブロックと同じX/Zで下3ブロックまで足場を探し、足元と頭上が空気で直下が solid の高さへ移動します。安全な高さが見つからない場合はワープしません。
...
自動収集、特殊アイテム、サーバー再起動後の永続性を実際のゲーム tick とパケット経路で検証します。
