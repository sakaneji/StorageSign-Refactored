...
登録済みStorageSignアイテムの一枚あたり保管数を単位としてマージする。最後の一枚を分解して
一部だけ移動することはなく、一枚分が収まらなければその一枚と内容はマージ元に残す。

| ID | テストケース | 期待結果 | レベル | 状態 |
|---|---|---|---|---|
| MRG-01 | 全StorageSignが数量上限内に収まる | 全内容を加算し、同じ枚数の空SSを手持ちへ返す | Unit | ✅ |
| MRG-02 | 複数枚の一部だけが数量上限内に収まる | 収まる枚数だけマージし、残りを登録済みSSとして維持する | Unit/Paper E2E | ✅ |
| MRG-03 | 一枚分未満の空きしかない | 看板とマージ元を変更しない | Unit | ✅ |
| MRG-04 | 部分マージ時にインベントリが満杯 | 未マージSSは手元に残し、空SSだけを足元へドロップする | Unit/Paper E2E | ✅ |
...
| BNR-06 | 起動時に旗メタ生成が一時失敗 | 次tickに1回だけ再試行する | Unit | ✅ |
| BNR-07 | 再試行も失敗 | 無限ループせず、旗搬出だけを無効化する | Unit | ✅ |
...
| BNR-12 | 1.21.11→26.1.2→26.2でワールド更新する | 同一処理で旗互換性を維持する | Upgrade E2E | ⏸️ |
...
| Potion表示 | 正規RegistryキーはPDCへ保存し、看板表示はVanilla標準幅90px以内の短縮形式にする |
...
| 自動搬送 | Hopper、Minecart、Double Chest、Dropper、Dispenser、Crafter、Chest Boatを対象とする |
...
自動化対象の残作業は、26.x環境の準備後にVER-04～06とBNR-12を実行することだけである。
